...
class ZenTirednessModifier: ModifierBase
{
    ...
    override void Init()
    {
        ...
        m_UnconThreshold        = PlayerBase.MAX_TIREDNESS * ((float)(100.0 - GetCHFSleepConfig().UnconPercentStart) / 100.0); // Converts uncon % start into a tiredness value/1000 (max tiredness)
    }
    ...
    override protected void OnTick(PlayerBase player, float deltaT)
    {
        ...
        float tiredDepth = (player.MAX_TIREDNESS * ((GetCHFSleepConfig().YawnPercentStart) / 100.0) - (player.MAX_TIREDNESS - tiredness); // How far into sleepiness are we past our yawn percentage?
        ...
        float yawnChance = GetCHFSleepConfig().YawnPercentChance + tiredDepthPenalty;
        float unconChance = GetCHFSleepConfig().UnconPercentChance + (tiredDepthPenalty / 2.0);
        ...
        if ((rand <= GetCHFSleepConfig().YawnPercentChance || fallUnconscious || m_UpdatesSinceLastYawn >= 100) && !player.m_FallUnconsciousFromTiredness && m_UpdatesSinceLastYawn > 1)
        {
