class ZenSleep_StimSyringe : Inventory_Base
{
    ...
    override void OnApply(PlayerBase player)
    {
        ...
        int penaltyPct = GetCHFSleepConfig().GetEnergyDrink("ZenSleep_StimSyringe").EnergyGained;
        if (penaltyPct == 0)
        {
            return;
        }
        ...
        // Drop used syringe
        ItemBase junk = ItemBase.Cast(GetGame().CreateObjectEx(GetCHFSleepConfig().UsedStimSyringeType, player.GetPosition(), ECE_PLACE_ON_SURFACE));
        if (junk)
        {
